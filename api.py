...
# Approximate dataset statistics used during training (StandardScaler)
AGE_MEAN = 50.0
AGE_STD = 20.0
...
class MultimodalFundusModel(nn.Module):
    def __init__(self, num_classes=8):
        ...
        fused_dim = 128 * 2 + 32  # left + right + tabular
...
def load_model() -> None:
    ...
    _models_loaded = True
    print(
        f"Model loaded from epoch {checkpoint.get('epoch', '?')} "
        f"(val_acc={checkpoint.get('val_acc', 'N/A'):.4f})"
        if isinstance(checkpoint.get("val_acc"), float)
        else f"Model loaded from epoch {checkpoint.get('epoch', '?')}"
    )
...
async def predict(
    ...
    gender: Optional[str] = Form(None, description="Patient gender: M/Male or F/Female (optional)"),
):
    """
    Classify ocular diseases from fundus image(s).

    - Send **1 image**: it is used as the left eye; a horizontal mirror is used as the right eye.
    - Send **2 images**: first = left eye, second = right eye.
    """
